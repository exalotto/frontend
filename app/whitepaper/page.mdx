...
counted in all lower categories.

The above Venn diagram shows us that we need to remove the number of winners of category $k_i$ from
all lower categories $k_j$ (with $2 \le j < i$) as many times as there are ways to choose $j$
elements out of $i$, or $i \choose j$, because that is how we can count all possible
