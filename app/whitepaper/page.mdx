...
category prize must be divided equally among them.
...
We can now go ahead with the full implementation.
...
number of 6-combinations in it, so a fair prize must also be proportional to the number of
6-combinations.
...
also use a nested `for` loop structure to scan all possible 2-, 3-, 4-, 5-, and 6-combinations of
the drawn numbers, the only difference being that this time there are exactly 6 numbers and we will
therefore find exactly 57 combinations, of which only one 6-combination.
...
Since the sets are not disjoint, the cardinality of the areas labeled `123`, `124`, `134`, and
