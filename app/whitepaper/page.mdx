...
The probability of an event is calculated by dividing the number of favorable cases by the number of total cases.

Assuming all tickets have 6 numbers for simplicity, the number of total cases is $90 \choose 6$.
Let's then calculate the number of favorable cases for matching _exactly_ 2 numbers, which is our
worst case because the 2-match category will often give us the largest amount of processing work.
...
etc. (We can rank our hashes and call $H_6$ the _6-hash_, $H_5$ the _5-hashes_, etc.)
...
Based on everything discussed so far we can write a Solidity function called `indexTicket` that
takes $k$ numbers between 1 and 90 inclusive (the numbers chosen by the player when buying the
...
hashes of all ranks to the respective number of rank-combinations ever played in the current round.
For example, the following 7-ticket:
...
- $({7 \choose 2} = 21)$ 2-combinations,
- $({7 \choose 3} = 35)$ 3-combinations,
- $({7 \choose 4} = 35)$ 4-combinations,
- $({7 \choose 5} = 21)$ 5-combinations,
- and $({7 \choose 6} = 7)$ 6-combinations

to count in the `index`.

To implement our `indexTicket` function we will first need a `getPrime(i)` function that returns the
$i$-th prime. Since we will only ever deal with the first 90 primes the implementation of `getPrime`
is fairly simple and will not be discussed here.
...
in the ticket. Let us start out with a nested `for` loop structure to scan said combinations:
...
### Determining the 6 Drawn Numbers

The ChainLink VRF can produce one or more 256-bit random words in exchange for a payment in `LINK`
tokens. The higher the number of returned words, the higher the `LINK` cost of the request. A single
256-bit word contains more than enough information to draw 6 random numbers. In fact we have:
...
We hereby discuss the implementation of a Solidity function that takes a single `uint256` value and
returns 6 unsigned integers between 1 and 90 inclusive **without repetitions**:
...
90 integers, production a random permutation based on our 256-bit source of randomness, and
returning the first 6. We can use the [Fisher-Yates shuffle][fisher-yates-shuffle] for that purpose.
...
The algorithm written so far has a major flaw: `randomness` doesn't have enough information to
scramble 90 numbers. We are dividing it by 90 at every step, so we have at most

$$
\frac{256}{log_2(90)} \approx \frac{256}{6.492} \approx 39
$$

random indices available, but we're trying to use 90 of them. However that is not a problem because
the Fisher-Yates algorithm changes the elements of the array in order and never changes an element
more than once, so we do not need to go through the whole array, we can simply stop at index 5:
...
We can further simplify the algorithm by writing our numbers to `numbers` directly rather than
modifying `source` and then copying the first 6 elements to `numbers`:
...
corresponding to th number of winning 6-combinations in each category.
...
of 6-combinations in it, so a fair prize for it must also be proportional to the number of
6-combinations.
...
these numbers. The function needs to pieces of input:
